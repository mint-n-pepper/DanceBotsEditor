...
void PrimitiveToSignal::convert(const QList<QObject*>& motorPrimitives,
  ...
  for(const QObject* const qo : motorPrimitives) {
    const MotorPrimitive* const mp = reinterpret_cast<const MotorPrimitive*>(qo);
...
void PrimitiveToSignal::processPrimitivesAtBeat(
  ...
  while(currentFrame < endFrame) {
    ...
    double beatFraction = static_cast<double>(currentFrame - startFrame)
      / nFrames;

    // get motor velocities, if there is a motor primitive:
    if(motorPrimitive) {
      double relativeBeat = (currentBeat - motorPrimitive->mPositionBeat)
        + beatFraction;
      getMotorVelocities(relativeBeat, motorPrimitive, data);
    }

    if(ledPrimitive) {
      double relativeBeat = (currentBeat - ledPrimitive->mPositionBeat)
        + beatFraction;
